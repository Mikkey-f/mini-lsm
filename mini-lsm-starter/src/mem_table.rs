...
impl MemTable {
    ...
    pub fn put(&self, _key: &[u8], _value: &[u8]) -> Result<()> {
        ...
        let val = Bytes::copy_from_slice(_value);

        // 2. 计算长度
        let len = key.len() + val.len();
        self.approximate_size.fetch_add(len, Ordering::Release);
    ...
    pub fn scan(&self, _lower: Bound<&[u8]>, _upper: Bound<&[u8]>) -> MemTableIterator {
        ...
        let entry = memtable_iter.with_iter_mut(|iter| {
            iter.next()
                .map(|entry| (entry.key().clone(), entry.value().clone()))
        });

        memtable_iter.with_item_mut(|item| {
